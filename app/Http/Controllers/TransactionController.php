...
class TransactionController extends Controller
{
    ...
    public function calculateTransactionAfterBalance()
    {
        try{
            ...
            return response()->json(['success'=>true,'message' => 'Transaction after balance calculated successfully','transaction_after_balance'=>$balanceByUserDate]);
    ...
    public function calculateMetrics(Request $request)
    {
        try {
            ...
            return response()->json(['success'=>true,'message'=>"transaction amount calculated successfully",
    ...
    {
        return Transaction::where('created_at', '>=', now()->subDays(30)->toDateString())
            ->where('created_at', '<=', now()->toDateString())
            ->where('trans_plaid_category_id', '!=', $excludedCategoryId)
    ...
    {
        return Transaction::whereDate('created_at', '>=', now()->subDays(30)->toDateString())
            ->where('trans_plaid_amount', '<', 0)
            ->count();
    }
    ...
    {
        return Transaction::where('created_at', '>=', now()->subDays(30)->toDateString())
        ->whereIn(DB::raw('DAYOFWEEK(created_at)'), [5, 6, 7]) // 5, 6, 7 represent Friday, Saturday, Sunday
        ->where('trans_plaid_amount', '<', 0)
        ->sum('trans_plaid_amount');
    }
